...
class GraphWin(tk.Canvas):
    ...
    def _onClick(self, e):
        ...
        if self._mouseCallback:
            self._mouseCallback(Point(e.x, e.y,-1))
...
class Point(GraphicsObject):
    def __init__(self, x, y, associated_number):
        GraphicsObject.__init__(self, ["outline", "fill"])
        self.setFill = self.setOutline
        self.x = float(x)
        self.y = float(y)
        self.num = int(associated_number)
        #self.label = Text(self, "", width=40)
    ...
    def clone(self):
        other = Point(self.x,self.y, self.num)
    ...
    '''def on_enter(self, event):
    	self.label.configure(text=str(self.num))
...
class _BBox(GraphicsObject):
    ...
    def getCenter(self):
        p1 = self.p1
        p2 = self.p2
        return Point((p1.x+p2.x)/2.0, (p1.y+p2.y)/2.0, -1)
...
class Circle(Oval):
    ...
    def __init__(self, center, radius):
        p1 = Point(center.x-radius, center.y-radius, -1)
        p2 = Point(center.x+radius, center.y+radius, -1)
...
def test():
    ...
    t = Text(Point(5,5,-1), "Centered Text")
    t.draw(win)
    p = Polygon(Point(1,1,-1), Point(5,3,-1), Point(2,7,-1))
    p.draw(win)
    e = Entry(Point(5,6,-1), 10)
    e.draw(win)
    win.getMouse()
    p.setFill("red")
    p.setOutline("blue")
